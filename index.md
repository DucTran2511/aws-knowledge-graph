# AWS Study Wiki ☁️

Welcome to your Map of Content (MOC) for Amazon Web Services. This is the entry point to your knowledge graph.

## 🏢 Services
*Atomic notes for individual AWS tools.*
- [[EC2]]
- [[VPC]]

## 🧠 Concepts
*Fundamental principles of cloud computing.*
- [[Availability Zones (AZ)]]
- [[Amazon Machine Images (AMI)]]
- [[EC2 Placement Groups]]
- [[EC2 Instance Store]]
- [[EC2 Storage Comparison]]
- [[AWS IP Addressing]]
- [[Elastic Block Store (EBS) Volumes]]
- [[Elastic File System (EFS)]]
- [[EBS Snapshots]]
- [[Elastic Network Interface (ENI)]]
- [[Subnets]]
- [[EC2 Hibernate]]
- [[Elastic Load Balancer (ELB)]]
- [[Application Load Balancer (ALB)]]
- [[Network Load Balancer (NLB)]]
- [[Gateway Load Balancer (GWLB)]]
- [[ELB Sticky Sessions]]
- [[ELB Cross-Zone Load Balancing]]
- [[ELB SSL TLS Certificates]]
- [[ELB Connection Draining]]
- [[Auto Scaling Group (ASG)]]
- [[Amazon RDS]]
- [[Amazon Aurora]]
- [[RDS Read Replicas]]
- [[RDS Read Replicas vs Multi-AZ]]
- [[Amazon ElastiCache]]
- [[Amazon Route 53]]
- [[Route 53 Simple Routing]]
- [[Route 53 Weighted Routing]]
- [[Route 53 Latency Routing]]
- [[Route 53 Failover Routing]]
- [[Route 53 Geolocation Routing]]
- [[Route 53 Geoproximity Routing]]
- [[Route 53 Multi-Value Routing]]
- [[Route 53 IP-Based Routing]]
- [[Route 53 Health Checks]]
- [[Amazon S3]]
- [[S3 Storage Classes]]
- [[S3 Encryption]]
- [[Amazon FSx]]
- [[AWS Snow Family]]
- [[AWS Storage Gateway]]
- [[AWS Transfer Family]]
- [[Amazon CloudFront]]
- [[AWS Global Accelerator]]
- [[Amazon SQS]]
- [[Amazon SNS]]
- [[Amazon Kinesis]]
- [[Amazon MQ]]
- [[Containers on AWS]]
- [[Docker]]
- [[Serverless on AWS]]
- [[AWS Lambda]]
- [[Amazon DynamoDB]]
- [[Amazon API Gateway]]
- [[AWS Step Functions]]
- [[Amazon Cognito]]
- [[Amazon CloudWatch]]
- [[AWS CloudTrail]]
- [[AWS Config]]
- [[Amazon EventBridge]]

## 🏗️ Architectures
*Design patterns and Well-Architected Framework principles.*

## 🎓 Certifications
*Study paths for specific exams.*

## 📥 Processing Queue
*Anything placed in the `raw/` directory that needs to be processed by the AI.*
